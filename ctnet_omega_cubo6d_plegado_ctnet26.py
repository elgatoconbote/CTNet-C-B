...
class HyperKernelND(nn.Module):
    """Kernel hiperradial 1D multi-rango usado por CTNet.

    CTNet 3.0 MAX mantiene la operación local reversible, pero evita que el
    kernel sea casi escalar: añade modulación por canal y mezcla de bajo rango.
    Así la capacidad entrenable escala con d sin introducir atención completa ni
    memoria creciente.
    """

    def __init__(self, channels: int, radii: Tuple[int, ...] = (1, 2, 4), rank: int | None = None):
        super().__init__()
        self.channels = channels
        self.radii = tuple(radii)
        self.rank = int(rank or min(32, max(4, channels // 8)))
        self.ctrl_logits = nn.Parameter(torch.zeros(len(self.radii), 3))
        self.scale = nn.Parameter(torch.tensor(1.0, dtype=torch.float32))
        self.channel_gain = nn.Parameter(torch.ones(channels, dtype=torch.float32))
        self.channel_bias = nn.Parameter(torch.zeros(channels, dtype=torch.float32))
        self.mix_u = nn.Parameter(0.01 * torch.randn(channels, self.rank, dtype=torch.float32))
        self.mix_v = nn.Parameter(0.01 * torch.randn(self.rank, channels, dtype=torch.float32))
        self.mix_scale = nn.Parameter(torch.tensor(0.05, dtype=torch.float32))
    ...
    def forward(self, z: torch.Tensor) -> torch.Tensor:
        ...
        gain = self.channel_gain.to(device=z.device, dtype=z.dtype)
        bias = self.channel_bias.to(device=z.device, dtype=z.dtype)
        y = y * gain + bias
        u = self.mix_u.to(device=z.device, dtype=z.dtype)
        v = self.mix_v.to(device=z.device, dtype=z.dtype)
        low = torch.matmul(torch.matmul(y, u), v) / math.sqrt(float(self.rank))
        y = y + self.mix_scale.to(device=z.device, dtype=z.dtype) * low
...
class CTNetFractal(nn.Module):
    """CTNet 3.0 MAX fractal reversible.

    Cada paso fractal posee su propio bloque. La inversa se aplica en orden
    inverso. Esto escala parámetros y profundidad real sin sacrificar la
    triangularidad reversible.
    """
    ...
    def __init__(self, d: int = 16, N: int = 64, fractal_steps: int = 4, latent_steps: int = 2):
        ...
        self.blocks = nn.ModuleList(
            [FractalCTBlock(d, SmallCTNetLatent(d // 2, steps=latent_steps)) for _ in range(self.fractal_steps)]
        )
        self.latent = SmallCTNetLatent(d // 2, steps=latent_steps)
        self.coh_tensor = CoherenceTensor(d=d, rank=max(4, min(32, d // 8)), beta=3.0)
